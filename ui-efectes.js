...
function findAvailablePhotos() {
    ...
    img.src = 'fotos/activo1.webp'; // imagen temporal mientras carga
...
function initScrollSpy() {
    ...
    function updateActiveSection() {
        ...
        // Force 'inicio' if at the top
        if (scrollPos < 100) {
            current = 'inicio';
        } else {
            sections.forEach(section => {
                // Use relative position to document
                const rect = section.getBoundingClientRect();
                const sectionTop = rect.top + scrollPos;
                
                // Earlier trigger for dades-programa section
                const triggerOffset = section.id === 'dades-programa' ? 400 : 150;
                
                if (scrollPos >= sectionTop - triggerOffset) {
...
// 7. Hero Depth Effect on Scroll
function initHeroDepth() {
    const hero = document.getElementById('inicio');
    if (!hero) return;

    // Create a dark overlay that we'll fade in on scroll
    const darkOverlay = document.createElement('div');
    darkOverlay.style.cssText = 'position:absolute;inset:0;background:black;opacity:0;pointer-events:none;transition:none;z-index:1;';
    ...
    window.addEventListener('scroll', function () {
        ...
        // Darken from 0 to 0.5 opacity
        darkOverlay.style.opacity = progress * 0.5;
    }, { passive: true });
}
...
function initFaqAnimations() {
    ...
    faqItems.forEach(detailsEl => {
        ...
        const content = detailsEl.querySelector(':scope > div');
        if (!summary || !content) return;

        // Configurar estado inicial FORZADO
        ...
        // Forzar reflow para asegurar estado inicial
        void content.offsetHeight;

        // Guardar altura real del contenido con margen extra
        const getContentHeight = () => {
            const tempDiv = content.cloneNode(true);
            tempDiv.style.cssText = 'position:absolute;visibility:hidden;height:auto;max-height:none;padding:24px;';
            document.body.appendChild(tempDiv);
            const height = tempDiv.scrollHeight;
            document.body.removeChild(tempDiv);
            return height + 50; // Añadir 50px extra para asegurar que todo el contenido sea visible
        };

        const targetHeight = getContentHeight();

        const animateOpen = () => {
            if (detailsEl.hasAttribute('open')) return;
            
            // Asegurar estado cerrado antes de animar
