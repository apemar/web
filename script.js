...
// Soluciona el bug de 100vh en iOS Safari (la barra del navegador no conta)
function setVH() {
    const vh = window.innerHeight * 0.01;
    document.documentElement.style.setProperty('--vh', `${vh}px`);
    // Actualitzar tamañ del spacer de l'Hero
    const heroSpacer = document.querySelector('div[style*="height: 100vh"]');
    if (heroSpacer) heroSpacer.style.height = window.innerHeight + 'px';
}
setVH();
window.addEventListener('resize', setVH, { passive: true });
window.addEventListener('orientationchange', function() {
    setTimeout(setVH, 200); // Esperem que acabe la rotació
}, { passive: true });
...
function initMobileMenu() {
    ...
    if (mobileMenuButton && mobileMenu) {
        // Toggle en clic del botó hamburguesa
        mobileMenuButton.addEventListener('click', function(e) {
            e.stopPropagation();
            mobileMenu.classList.toggle('hidden');
            // Actualitzar aria per accessibilitat
            const isOpen = !mobileMenu.classList.contains('hidden');
            mobileMenuButton.setAttribute('aria-expanded', isOpen);
        });

        // Tancar menú al fer clic en un dels enllaços
...
function initSmoothScroll() {
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (target) {
                // Special case: #inicio scrolls to top since Hero is fixed
                let offsetTop;
                const isMobile = window.innerWidth < 768;
                const navOffset = isMobile ? 64 : 80;
                if (targetId === '#inicio') {
                    offsetTop = 0;
                } else if (targetId === '#dades-programa') {
                    offsetTop = target.getBoundingClientRect().top + window.scrollY - (isMobile ? 120 : 240);
                } else {
                    offsetTop = target.getBoundingClientRect().top + window.scrollY - navOffset;
                }
                window.scrollTo({
                    top: offsetTop,
                    behavior: 'smooth'
...
// Funció per a traduir títols de transparència (Espanyol -> Valencià)
function formatTransparencyTitle(title) {
    if (!title) return 'Document sense títol';
    return title
        .replace(/Memoria año/gi, 'Memòria any')
        .replace(/Balance año/gi, 'Balanç any')
        .replace(/Subvención año/gi, 'Subvenció any')
        .replace(/Memoria/gi, 'Memòria')
        .replace(/Balance/gi, 'Balanç')
        .replace(/Subvención/gi, 'Subvenció')
        .replace(/año/gi, 'any');
}
...
function displayMemoriasFromAppsScript(memorias) {
    ...
    const html = sortedMemorias.map((memoria, index) => {
        ...
        const dateStr = memoria.modifiedTime || memoria.dateUpdated ? 
            new Date(memoria.modifiedTime || memoria.dateUpdated).toLocaleDateString('ca-ES', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit'
            }) : 
            memoria.createdTime || memoria.dateCreated ? 
            new Date(memoria.createdTime || memoria.dateCreated).toLocaleDateString('ca-ES', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit'
            }) : 
            memoria.date ? 
            new Date(memoria.date).toLocaleDateString('ca-ES', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit'
            }) : 
        ...
        return `
            <div class="bg-white border-l-4 ${color.border} rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center mb-4">
                    ...
                    <div>
                        <h5 class="font-semibold text-gray-800">${formatTransparencyTitle(memoria.title || memoria.name)}</h5>
...
function displayBalanceFromAppsScript(balance) {
    ...
    if (balance.length === 0) {
        ...
        container.innerHTML = `
            <div class="col-span-full text-center py-8">
                ...
                <p class="text-gray-500">No hi ha documents de balanç disponibles en aquest moment</p>
            </div>
        `;
        return;
    }
    ...
    const html = balance.map((item, index) => {
        ...
        const dateStr = item.modifiedTime || item.dateUpdated ? 
            new Date(item.modifiedTime || item.dateUpdated).toLocaleDateString('ca-ES', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit'
            }) : 
            item.createdTime || item.dateCreated ? 
                new Date(item.createdTime || item.dateCreated).toLocaleDateString('ca-ES', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit'
                }) : 
        ...
        return `
            <div class="bg-white border-l-4 ${color.border} rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center mb-4">
                    ...
                    <div>
                        <h5 class="font-semibold text-gray-800">${formatTransparencyTitle(item.title || item.name)}</h5>
                        <p class="text-sm text-gray-600">Publicat: ${dateStr}</p>
                    </div>
                </div>
...
function displaySubvencionesFromAppsScript(subvenciones) {
    ...
    if (subvenciones.length === 0) {
        ...
        container.innerHTML = `
            <div class="col-span-full text-center py-8">
                <i class="fas fa-info-circle text-4xl text-gray-400 mb-4"></i>
                <p class="text-gray-500">No hi ha documents de subvencions disponibles en aquest moment</p>
            </div>
        `;
        return;
    }
...
function displayMemoriasFromDrive(files) {
    ...
    if (sortedFiles.length === 0) {
        container.innerHTML = `
            <div class="col-span-full text-center py-8">
                <i class="fas fa-info-circle text-4xl text-gray-400 mb-4"></i>
                <p class="text-gray-500">No hi ha documents de memòria disponibles en aquest moment</p>
            </div>
        `;
        return;
    }
...
function displayMenuFromDrive(files) {
    ...
    if (files.length === 0) {
        menuContent.innerHTML = `
            <div class="text-center py-8">
                <i class="fas fa-info-circle text-4xl text-gray-400 mb-4"></i>
                <p class="text-gray-500">No hi ha menús disponibles per a aquest mes</p>
            </div>
        `;
        return;
    }
    ...
    const displayName = cleanName || 'Menú del dia';
    const formattedDate = new Date(latestFile.dateUpdated || latestFile.dateCreated).toLocaleDateString('ca-ES', {
        day: 'numeric',
        month: 'long',
        year: 'numeric'
    });
